...
LOG_MODULE_REGISTER(xgzp6897d, CONFIG_SENSOR_LOG_LEVEL);
/**
 * @brief reads a set value from a reg of choice
 *
 * @param[in] dev  device instance. Must not be NULL.
 * @param[in] reg  reg you want to change
 * @param[out] val  a variable that will contain the contents read
 * @param[in] len  length of the reg being read
 *
 * @return  -errno   Standard negative error code on failure.
 */
static int xgzp6897d_read_reg(const struct device *dev, uint8_t reg, uint8_t *val, uint8_t len)
{
...
/**
 * @brief writes a set value to a reg of choice
 *
 * @param[in] dev  device instance. Must not be NULL.
 * @param[in] reg  reg you want to change
 ...
 * @return -errno   Standard negative error code on failure.
 */
static int xgzp6897d_write_reg(const struct device *dev, uint8_t reg, uint8_t val)
{
	const xgzp6897d_config_t *cfg = (const xgzp6897d_config_t *)dev->config;
 ...
 * @return -errno   Standard negative error code on failure.
 */
static int xgzp6897d_write_mask(const struct device *dev, uint8_t reg, uint8_t mask, uint8_t val)
{
	uint8_t temp;
	int ret = xgzp6897d_read_reg(dev, reg, &temp, sizeof(temp));
	...
	return xgzp6897d_write_reg(dev, reg, temp);
}

/**
 * @brief Verify communication with the DualPortDevice.
 *
 * @details Reads the WHOAMI reg and checks that it matches the configured
 * I2C address.
 *
 * @param[in] dev  device instance. Must not be NULL.
 *
 ...
 * @return Negative errno from the reg read operation on failure.
 */
static int xgzp6897d_whoami(const struct device *dev)
{
	...
	int ret = xgzp6897d_read_reg(dev, XGZP6897D_REG_WHOAMI, &temp, sizeof(temp));
	if (ret != 0) {
		return ret;
	}

	printk("WHO AM I Reg reads  0x%02X  \n", temp);
	return 0;
}
 ...
 *
 * @details Reads 2 regs of tempurature data then uses preset values stored in a seperate
 * reg for calculating the true tempurature value in degrees celcicus
...
static int xgzp6897d_get_temperature_mC(const struct device *dev, uint32_t *temp_C)
{
	...
	xgzp6897d_read_reg(dev, XGZP6897D_REG_TEMPURATURE_H, tmp, sizeof(tmp));
	uint16_t raw = ((uint16_t)tmp[0] << 8) | (uint16_t)tmp[1];

	xgzp6897d_read_reg(dev, XGZP6897D_REG_CALCOE, &byte1_raw, sizeof(byte1_raw));
	xgzp6897d_read_reg(dev, XGZP6897D_REG_CALCOE_2, &byte2, sizeof(byte2));
 ...
 *
 * @details Reads 3 regs of pressure data then uses preset PMIN and PMAX for
...
static int xgzp6897d_get_diff_pressure_mPa(const struct device *dev, int32_t *press_mPa)
{
	...
	uint8_t tmp[3];
	xgzp6897d_read_reg(dev, XGZP6897D_REG_PRESSURE_H, tmp, sizeof(tmp));
 ...
 * @retval -ERANGE If the WHOAMI value does not match the configured address.
 * @return Negative errno from reg access or range configuration on failure.
 */
