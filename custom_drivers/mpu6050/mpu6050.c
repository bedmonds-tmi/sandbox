...
int mpu6050_init(mpu6050_device_t *dev)
{
	...
	int ret = mpu6050_set_gyro_fs(dev, dev->config.gyro_fs);
	if (ret != 0)
	{
	...
	dev->accel_filtered = accel;
	dev->gyro_filtered = gyro;
...
int mpu6050_get_accel(mpu6050_device_t *dev, mpu6050_accel_t *raw)
{
	...
	raw->z = (int16_t)sys_get_be16(&tmp[4]);

	dev->accel_filtered.x = iir_update(dev->accel_filtered.x, raw->x);
	dev->accel_filtered.y = iir_update(dev->accel_filtered.y, raw->y);
	dev->accel_filtered.z = iir_update(dev->accel_filtered.z, raw->z);
...
int mpu6050_get_gyro(mpu6050_device_t *dev, mpu6050_gyro_t *raw)
{
	...
	dev->gyro_filtered.x = iir_update(dev->gyro_filtered.x, raw->x);
	dev->gyro_filtered.y = iir_update(dev->gyro_filtered.y, raw->y);
	dev->gyro_filtered.z = iir_update(dev->gyro_filtered.z, raw->z);
...
int mpu6050_set_gyro_fs(mpu6050_device_t *dev, mpu6050_gyro_fs_t range)
{
	...
	int ret = write_mask(dev, MPU6050_REG_GYRO_CONFIG, MPU6050_MASK_GYRO_CONFIG_FS_SEL, range);
	if (ret != 0)
	{
		return ret;
	}
...
int mpu6050_set_accel_fs(mpu6050_device_t *dev, mpu6050_accel_fs_t range)
{
	...
	int ret =
		write_mask(dev, MPU6050_REG_ACCEL_CONFIG, MPU6050_MASK_ACCEL_CONFIG_AFS_SEL, range);
	if (ret != 0)
	{
		return ret;
	}
...
int mpu6050_get_temp_C(mpu6050_device_t *dev, float *tmp_C)
{
	uint8_t val[2];
	read_reg(dev, MPU6050_REG_TEMP_OUTH, val, sizeof(val));
	int16_t raw = (int16_t)(((uint16_t)val[0] << 8) | val[1]);
	*tmp_C = (float)raw / 340.0f;
	*tmp_C += 36.53f;
	return 0;
}